...
export default function BirthdayCake({ onDone, onBack }: Props) {
  ...
  const startMic = useCallback(async () => {
    try {
      ...
      const tick = () => {
        ...
        for (let i = start; i < end; i++) { sum += data[i]; c++; }
        const avg = c ? sum / c : 0;
        if (avg >= THRESHOLD) {
          quiet = 0;
  ...
  const blowOnce = () => {
    ...
    setLeaning(true);
    apply(streak.current);
    setTimeout(() => { if (!doneRef.current) setLeaning(false); }, 280);
  };
  ...
  const startHold = () => {
    ...
    const step = () => {
      ...
      apply(streak.current);
      if (streak.current < FULL) holdRaf.current = requestAnimationFrame(step);
    };
  ...
  return (
    <div className="fixed inset-0 z-20 flex flex-col items-center justify-center bg-hearts px-5">
      {onBack && (
        <button onClick={onBack} className="absolute top-5 left-4 z-30 text-xs text-white/80 px-3 py-1.5 rounded-full bg-white/20 border border-white/30 active:scale-95">
          ← Back
        </button>
      )}

      <div className="cream-card w-full max-w-[340px] rounded-2xl p-6 text-center" style={{ animation: "cardIn 0.5s ease-out both" }}>
        <p className="text-xs tracking-[0.2em] uppercase text-[#c9184a] mb-1">Birthday cake for you</p>
        ...
        <div className="relative mx-auto select-none" style={{ width: 220, height: 190 }}>
          ...
          <span className="absolute text-sm" style={{ left: "38%", bottom: "3.5rem" }}>🍒</span>
          <span className="absolute text-sm" style={{ left: "58%", bottom: "3.5rem" }}>🍒</span>

          {(leaning || holding) && [0, 1, 2].map((i) => (
            <div key={i} className="absolute left-0 right-0 pointer-events-none" style={{ top: `${28 + i * 8}%`, height: 2, background: "linear-gradient(90deg, transparent, rgba(200,220,255,0.55), transparent)", animation: `windLine 0.35s ease-in-out ${i * 0.08}s infinite` }} />
          ))}

          {xs.map((x, i) => (
            <div key={i} className="absolute flex flex-col items-center" style={{ left: `${x}%`, bottom: "7.2rem", transform: "translateX(-50%)" }}>
              {!extinguished[i] && (
                <div className="origin-bottom" style={{ transform: leaning || holding ? `rotate(${10 + i * 3}deg) scaleY(0.72)` : "rotate(0) scaleY(1)", transition: "transform 0.12s ease-out" }}>
                  <div className="w-3 h-5 rounded-full mx-auto" style={{ background: "radial-gradient(ellipse at 50% 70%,#fffef5,#ffe070 40%,#ff8020)", animation: "flicker 0.28s ease-in-out infinite alternate", boxShadow: "0 0 10px rgba(255,140,40,0.75)" }} />
                </div>
              )}
              {extinguished[i] && (
                <div className="w-2 h-3 rounded-full mx-auto" style={{ background: "rgba(160,140,150,0.45)", animation: "smokeUp 1.4s ease-out infinite" }} />
              )}
        ...
        {!done && (
          <div className="mt-5 space-y-3">
            <div className="w-40 h-2 mx-auto rounded-full bg-[#ffe4ec] overflow-hidden">
              <div className="h-full rounded-full bg-gradient-to-r from-[#f0a8bc] to-[#c45c6a]" style={{ width: `${progress * 100}%`, transition: "width 70ms linear" }} />
            </div>
            <p className="text-[11px] text-[#8a6870]">
              {progress < 0.33 ? "Blow gently… first candle" : progress < 0.66 ? "Keep going… second candle" : "Almost there… last candle"}
            </p>
            <div className="flex flex-col items-center gap-2">
              <button
                ...
                style={{ background: holding ? "linear-gradient(135deg, #b84d5c, #9a3a48)" : "linear-gradient(135deg, #d4788a, #c45c6a)", boxShadow: "0 4px 14px rgba(196,92,106,0.35)" }}
              >
                {holding ? "💨 Blowing…" : "🌬️ Hold to blow"}
              </button>
              {mic === "off" && (
                <button onClick={startMic} className="text-xs text-[#8a6870] underline">or use microphone (keep blowing)</button>
              )}
              {mic === "on" && <p className="text-xs text-[#c9184a]">Mic on — blow into the phone 🎤</p>}
              {mic === "denied" && <p className="text-xs text-[#8a6870]">Mic blocked — hold the button instead</p>}
            </div>
          </div>
        )}

        {done && <p className="mt-5 text-sm text-[#c9184a]" style={{ animation: "cardIn 0.5s ease-out" }}>Wish granted ✨</p>}
      </div>
    </div>
  );
}
